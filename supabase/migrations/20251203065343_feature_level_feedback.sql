...
-- Policies
drop policy if exists "Users can read their own feature feedback" on user_feature_feedback;
create policy "Users can read their own feature feedback"
  on user_feature_feedback for select
  using (auth.uid() = user_id);

drop policy if exists "Users can update their own feature feedback" on user_feature_feedback;
create policy "Users can update their own feature feedback"
  on user_feature_feedback for update
  using (auth.uid() = user_id);

drop policy if exists "Users can insert their own feature feedback" on user_feature_feedback;
