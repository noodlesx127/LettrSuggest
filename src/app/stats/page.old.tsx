...
export default function StatsPage() {
  ...
  >([]);
  const [tasteProfileData, setTasteProfileData] = useState<Awaited<
    ReturnType<typeof buildTasteProfile>
  > | null>(null);
  ...
  const [sourceReliability, setSourceReliability] = useState<
    Array<{ source: string; total: number; positive: number; hitRate: number }>
  >([]);
  ...
  const [reasonAcceptance, setReasonAcceptance] = useState<
    Array<{ reason: string; total: number; positive: number; hitRate: number }>
  >([]);
  ...
  useEffect(() => {
    async function fetchExplorationStats() {
      ...
      try {
        ...
        if (pairwiseError) {
          console.error(
            "[Stats] Error fetching pairwise events:",
            pairwiseError,
          );
  ...
  useEffect(() => {
    async function fetchFeedbackSummary() {
      ...
      try {
        ...
        const positive =
          data?.filter((r) => r.feedback_type === "positive").length ?? 0;
        const negative =
          data?.filter((r) => r.feedback_type === "negative").length ?? 0;
        ...
        data?.forEach((row) => {
          const reasons: string[] = Array.isArray((row as any).reason_types)
            ? (row as any).reason_types
            : [];
        ...
        data?.forEach((row) => {
          const sources: string[] = Array.isArray(
            (row as any).recommendation_sources,
          )
            ? (row as any).recommendation_sources
            : [];
          const isPos = row.feedback_type === "positive";
        ...
        const bySourceConsensus = new Map<
          string,
          {
            high: { pos: number; total: number };
            medium: { pos: number; total: number };
        ...
        data?.forEach((row) => {
          const sources: string[] = Array.isArray(
            ...
            ? (row as any).recommendation_sources
            : [];
          const level = (row as any).consensus_level as
            | "high"
            | "medium"
            | "low"
            | null;
  ...
  const filteredFilms = useMemo(() => {
    ...
    const watched = films.filter(
      (f) => (f.watchCount ?? 0) > 0 || f.rating != null || !!f.lastDate,
    );
  ...
  useEffect(() => {
    if (!uid || !filteredFilms.length) {
      console.log("[Stats] Skipping TMDB load:", {
        uid,
    ...
    async function loadTmdbDetails() {
      console.log("[Stats] Starting TMDB details load", {
        uid,
        filmCount: filteredFilms.length,
      });
      ...
      try {
        ...
        while (true) {
          ...
          if (mappingError) {
            console.error("[Stats] Error fetching mappings page:", {
              from,
              error: mappingError,
            });
        ...
        const relevantMappings = allMappings.filter((m) =>
          filteredUris.has(m.uri),
        );
        console.log(
          "[Stats] Relevant mappings:",
        ...
        setMappingCoverage({
          mapped: relevantMappings.length,
          total: filteredUris.size,
        });
        ...
        console.log(
          "[Stats] Fetching cached TMDB details for",
          tmdbIds.length,
          "IDs",
        );
        ...
        for (let i = 0; i < tmdbIds.length; i += batchSize) {
          ...
          console.log(
            `[Stats] Fetching batch ${i / batchSize + 1}:`,
            batch.length,
            "IDs",
          );
        ...
        for (const [, data] of detailsMap) {
          ...
          if (data.credits?.cast?.length || data.credits?.crew?.length)
            withCredits++;
          if (data.keywords?.keywords?.length || data.keywords?.results?.length)
            withKeywords++;
        }
  ...
  const stats = useMemo(() => {
    ...
    // Most watched film
    const mostWatched = filteredFilms.reduce(
      (max, f) => ((f.watchCount ?? 0) > (max.watchCount ?? 0) ? f : max),
      filteredFilms[0],
    );
    ...
    for (const f of filteredFilms) {
      ...
      if (details?.production_companies) {
        ...
        const isIndie = companies.some((c) =>
          ["A24", "Neon", "Annapurna", "Searchlight", "Focus"].some((n) =>
            c.name.includes(n),
          ),
        );
        const isMajor = companies.some((c) =>
          [
            "Universal",
            "Warner",
            "Disney",
    ...
    const dislikedFilmsCount = filteredFilms.filter(
      (f) => f.rating != null && f.rating <= 1.5 && !f.liked,
    ).length;
    ...
    const avoidanceOverrides = {
      genres: tasteProfileData.avoidGenres
        .filter((g) =>
          tasteProfileData.watchlistGenres.some((wg) => wg.name === g.name),
        )
        .map((g) => ({
          ...g,
          watchlistCount:
            tasteProfileData.watchlistGenres.find((wg) => wg.name === g.name)
              ?.count || 0,
        })),
      keywords: tasteProfileData.avoidKeywords
        .filter((k) =>
          tasteProfileData.watchlistKeywords.some((wk) => wk.name === k.name),
        )
        .map((k) => ({
          ...k,
          watchlistCount:
            tasteProfileData.watchlistKeywords.find((wk) => wk.name === k.name)
              ?.count || 0,
        })),
      directors: tasteProfileData.avoidDirectors
        .filter((d) =>
          tasteProfileData.watchlistDirectors.some((wd) => wd.name === d.name),
        )
        .map((d) => ({
          ...d,
          watchlistCount:
            tasteProfileData.watchlistDirectors.find((wd) => wd.name === d.name)
              ?.count || 0,
        })),
    };
    ...
    return {
      ...
      watchHours: Math.round(
        filteredFilms.reduce((acc, f) => acc + ((f as any).runtime ?? 0), 0) /
          60,
      ),
      ratingsBuckets,
      yearCounts,
      ...
      decadeCounts,
      decades,
      topYears: Array.from(byYear.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
      ...
      absoluteFavorites: filteredFilms.filter(
        (f) => f.rating === 5 || ((f.rating ?? 0) >= 4.5 && f.liked),
      ).length,
      highlyRatedCount: filteredFilms.filter(
        (f) => f.rating != null && f.rating >= 4,
      ).length,
      lowRatedButLikedCount: filteredFilms.filter(
        (f) => f.rating != null && f.rating < 3 && f.liked,
      ).length,
      avgRating,
      ...
      studioPreference,
      topDecades: tasteProfileData.topDecades.map((d) => [
        `${d.decade}s`,
        d.weight,
      ]),
      ...
      medianWatchlistAge:
        watchlistAges.length > 0
          ? watchlistAges.sort((a, b) => a - b)[
              Math.floor(watchlistAges.length / 2)
            ]
          : 0,
      avgWatchlistAge:
        watchlistAges.length > 0
          ? totalWatchlistAgeDays / watchlistAges.length
          : 0,
  ...
  const feedbackAnalytics = useMemo(() => {
    ...
    const positives = feedbackRows.filter(
      (r) => r.feedback_type === "positive",
    );
    ...
    positives.forEach((row) => {
      ...
      (features.genres || []).forEach((g: any) =>
        addCount(genreCounts, g?.name ?? g),
      );
      (features.directors || []).forEach((d: any) =>
        addCount(directorCounts, d?.name ?? d),
      );
      (features.actors || []).forEach((a: any) =>
        addCount(actorCounts, a?.name ?? a),
      );
      (features.keywords || []).forEach((k: any) =>
        addCount(keywordCounts, k?.name ?? k),
      );
    });

    const totalGenreCounts = Array.from(genreCounts.values()).reduce(
      (s, v) => s + v,
      0,
    );
    const topGenreCount =
      totalGenreCounts > 0 ? Math.max(...genreCounts.values()) : 0;
    ...
    return {
      ...
      topGenreShare:
        totalGenreCounts > 0 ? topGenreCount / totalGenreCounts : 0,
    };
  }, [feedbackRows]);

  const regretStats = useMemo(() => {
    if (!feedbackRows || feedbackRows.length === 0 || tmdbToFilm.size === 0)
      return null;
    const negatives = feedbackRows.filter(
      (r) => r.feedback_type === "negative",
    );
  ...
  useEffect(() => {
    ...
    console.log(
      "[TasteProfile] Will show Taste Profile section:",
      willShowTasteProfile,
    );
    if (!willShowTasteProfile) {
      console.warn(
        "[TasteProfile] ⚠️ Taste Profile will NOT show - no genre data!",
      );
    ...
    return (
      <AuthGate>
        ...
        <p className="text-gray-600">
          No data yet. Import your Letterboxd data to see stats.
        </p>
      </AuthGate>
    );
  }
    ...
    series: [
      {
        type: "pie",
        radius: ["40%", "70%"],
        data: stats.topGenres.map(({ name, count }) => ({
          value: count,
          name,
        })),
        label: { show: true },
    ...
    <AuthGate>
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          {stats.rewatchedCount > 0 && (
            <p className="text-xs text-gray-500 mt-1">
              {stats.rewatchedCount} rewatched
            </p>
          )}
        </div>
        <div className="bg-white border rounded-lg p-4">
        ...
        <div className="bg-white border rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-600">
              {stats.metadataCoverage.total} titles checked (watch history +
              watchlist)
            </span>
          </div>
          <p className="text-xs text-gray-600 mb-3">
            Quality gates downrank missing metadata; higher coverage =
            higher-confidence picks.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs text-gray-700">
            <div className="flex items-center gap-1">
              <span className="w-24 text-gray-500">Posters</span>
              <strong>
                {Math.round(stats.metadataCoverage.poster * 100)}%
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-24 text-gray-500">Backdrops</span>
              <strong>
                {Math.round(stats.metadataCoverage.backdrop * 100)}%
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-24 text-gray-500">Overviews</span>
              <strong>
                {Math.round(stats.metadataCoverage.overview * 100)}%
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-24 text-gray-500">Trailers</span>
              <strong>
                {Math.round(stats.metadataCoverage.trailer * 100)}%
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-24 text-gray-500">Votes ≥50</span>
              <strong>
                {Math.round(stats.metadataCoverage.voteCount * 100)}%
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-24 text-gray-500">Rating ≥6.0</span>
              <strong>
                {Math.round(stats.metadataCoverage.tmdbRating * 100)}%
              </strong>
            </div>
          </div>
        </div>
      ...
      {stats.consensus && stats.consensus.total > 0 && (
        <div className="bg-white border rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-600">
              {stats.consensus.total} titles (watch history + watchlist)
            </span>
          </div>
          <p className="text-xs text-gray-600 mb-3">
            Higher vote counts = steadier ratings; we soften quality penalties
            when consensus is strong.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-gray-700">
            <div className="flex items-center gap-1">
              <span className="w-28 text-gray-500">Strong (200+ votes)</span>
              <strong>
                {Math.round(
                  (stats.consensus.strong / stats.consensus.total) * 100,
                )}
                %
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-28 text-gray-500">Moderate (50-199)</span>
              <strong>
                {Math.round(
                  (stats.consensus.moderate / stats.consensus.total) * 100,
                )}
                %
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-28 text-gray-500">Weak (&lt;50)</span>
              <strong>
                {Math.round(
                  (stats.consensus.weak / stats.consensus.total) * 100,
                )}
                %
              </strong>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-28 text-gray-500">Missing</span>
              <strong>
                {Math.round(
                  (stats.consensus.missing / stats.consensus.total) * 100,
                )}
                %
              </strong>
            </div>
          ...
          {stats.consensus.totalVotes > 0 && (
            <div className="text-xs text-gray-600 mt-2">
              Avg vote count:{" "}
              {Math.round(
                stats.consensus.totalVotes / stats.consensus.total,
              )}{" "}
            </div>
          )}
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-amber-800">
                  Incomplete Film Enrichment
                </h3>
                <p className="text-sm text-amber-700 mt-1">
                  Only {mappingCoverage.mapped} of {mappingCoverage.total} films
                  (
                  {Math.round(
                    (mappingCoverage.mapped / mappingCoverage.total) * 100,
                  )}
                  %) have TMDB data. This affects Taste Profile, Suggestions,
                  and detailed stats.
                </p>
                <p className="text-sm text-amber-700 mt-1">
                  ...
                  to complete enrichment and unlock full features.
                </p>
              </div>
            </div>
          </div>
        ...
        <>
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-6 mb-6">
            ...
            <p className="text-sm text-gray-600 mb-4">
              These weighted preferences drive your movie suggestions. Higher
              weights mean stronger influence.
            </p>
            ...
            <div className="mb-4">
              <h3 className="font-medium text-gray-900 mb-2 text-sm">
                Top Genre Preferences (Weighted)
              </h3>
            ...
            {stats.topKeywords.length > 0 && (
              <div className="mb-4">
                <h3 className="font-medium text-gray-900 mb-2 text-sm">
                  Top Themes & Keywords (Weighted)
                </h3>
                <div className="flex flex-wrap gap-2">
                  {stats.topKeywords
                    .slice(0, 12)
                    .map(({ name: keyword, weight }) => {
                      const strength =
                        weight >= 3.0
                          ? "strong"
                          : weight >= 1.5
            ...
            {stats.topDirectors.length > 0 && (
              <div className="mb-4">
                <h3 className="font-medium text-gray-900 mb-2 text-sm">
                  Favorite Directors (Weighted by Ratings)
                </h3>
                <div className="flex flex-wrap gap-2">
                  {stats.topDirectors.map(({ name, weight, count }) => (
                    <span
                      key={name}
                      className="px-3 py-1 rounded-full text-xs font-medium bg-blue-500 text-white"
                    >
            ...
            {stats.topActors.length > 0 && (
              <div className="mb-4">
                <h3 className="font-medium text-gray-900 mb-2 text-sm">
                  Favorite Actors (Weighted by Ratings)
                </h3>
                <div className="flex flex-wrap gap-2">
                  {stats.topActors.map(({ name, weight, count }) => (
                    <span
                      key={name}
                      className="px-3 py-1 rounded-full text-xs font-medium bg-purple-500 text-white"
                    >
                      {name} ({weight.toFixed(1)} across {count} films)
                    </span>
                  ))}
            ...
            {stats.topStudios.length > 0 && (
              <div>
                ...
                <p className="text-xs text-gray-600 mb-2">
                  Production companies whose films resonate with you
                </p>
                  ...
                  <div className="mb-3 bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="flex gap-2 h-8">
                      <div
                        ...
                        style={{
                          width: `${(stats.studioPreference.indie / stats.studioPreference.total) * 100}%`,
                        }}
                      >
                        {stats.studioPreference.indie > 0 &&
                          `Indie ${stats.studioPreference.indie.toFixed(1)}`}
                      </div>
                      <div
                        className="bg-blue-500 flex items-center justify-center text-white text-xs font-medium rounded transition-all"
                        style={{
                          width: `${(stats.studioPreference.major / stats.studioPreference.total) * 100}%`,
                        }}
                      >
                        {stats.studioPreference.major > 0 &&
                          `Major ${stats.studioPreference.major.toFixed(1)}`}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2">
                  {stats.topStudios.slice(0, 10).map((entry) => {
                    ...
                    return (
                      <span
                        key={studio}
                        className="px-3 py-1 rounded-full text-xs font-medium bg-amber-500 text-white"
                      >
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            {stats.topDecades && stats.topDecades.length > 0 && (
              <div className="bg-white border rounded-lg p-4">
                ...
                <p className="text-xs text-gray-600 mb-3">
                  Decades you&apos;ve watched most. Not used to limit
                  suggestions—we&apos;ll recommend great films from any era!
                </p>
                <div className="space-y-2">
                  {stats.topDecades.map((entry) => {
                    const [decade, weight] = entry as [string, number];
                    const percentage =
                      (weight / (stats.topDecades[0][1] as number)) * 100;
                    return (
      ...
      {stats.mostWatched && (stats.mostWatched.watchCount ?? 0) > 1 && (
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-4 mb-6">
          ...
          <p className="text-lg font-bold text-purple-900">
            ...
            {stats.mostWatched.year && `(${stats.mostWatched.year})`}
          </p>
          <p className="text-sm text-purple-700">
            Watched {stats.mostWatched.watchCount} times
          </p>
        </div>
      )}
      ...
      {loadingDetails ? (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-blue-800">
            Loading detailed stats (actors, directors, genres)...
          </p>
        ...
        <>
          ...
          {stats.topSubgenresList && stats.topSubgenresList.length > 0 && (
            <div className="bg-white border rounded-lg p-4 mb-6">
              <h2 className="font-semibold text-gray-900 mb-3">
                Top Specific Interests (Sub-genres)
              </h2>
              <p className="text-sm text-gray-500 mb-3">
                Identified from your viewing patterns using TMDB keywords and
                genres.
              </p>
              <div className="flex flex-wrap gap-2">
                {stats.topSubgenresList.map((item) => {
                  ...
                  return (
                    <div
                      key={item.name}
                      className="bg-indigo-50 border border-indigo-100 rounded px-3 py-2 flex flex-col min-w-[120px]"
                    >
                      <span className="font-medium text-indigo-900 capitalize text-sm">
                        {name}
                      </span>
      ...
      {stats && (
        <div className="bg-white border rounded-lg p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {feedbackSummary && feedbackSummary.total > 0 && (
              <div className="bg-blue-50 rounded p-3">
                ...
                <div className="text-xs text-gray-500 mt-1">
                  {feedbackSummary.positive} 👍 vs {feedbackSummary.negative} 👎
                  ({feedbackSummary.total} feedback)
                  <br />
                  Per-source reliability below once enough samples exist.
                </div>
              </div>
            )}
          </div>
          ...
          {consensusAcceptance &&
            consensusAcceptance.high.total +
              consensusAcceptance.medium.total +
              consensusAcceptance.low.total >
              0 && (
                    ...
                    return (
                      <div key={level} className="bg-gray-50 px-2 py-2 rounded">
                        ...
                        <div className="text-gray-800 mt-1">
                          {rate != null
                            ? `${rate.toFixed(0)}% (${bucket.pos}/${bucket.total})`
                            : "—"}
                        </div>
                      </div>
                    );
          ...
          {sourceReliability.length > 0 && (
            <div className="bg-white border rounded p-3 mt-3">
              <div className="text-sm font-medium text-gray-900 mb-2">
                Per-Source Reliability (min 3 samples)
              </div>
                ...
                {sourceReliability.map((entry) => (
                  ...
                  >
                    ...
                    <span className="text-gray-600">
                      {(entry.hitRate * 100).toFixed(0)}% ({entry.positive}/
                      {entry.total})
                    </span>
                  </div>
                ))}
          ...
          {sourceReliabilityRecent.length > 0 && (
            <div className="bg-white border rounded p-3 mt-3">
              <div className="text-sm font-medium text-gray-900 mb-2">
                Per-Source Reliability (Last 90d, min 3 samples)
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-xs text-gray-700">
          ...
          {reasonAcceptance.length > 0 && (
            <div className="bg-white border rounded p-3 mt-3">
              <div className="text-sm font-medium text-gray-900 mb-2">
                Acceptance by Reason Type (min 5 samples)
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-xs text-gray-700">
          ...
          {feedbackAnalytics && feedbackAnalytics.positiveCount >= 3 && (
            <div className="bg-white border rounded p-3 mt-3">
              <div className="text-sm font-medium text-gray-900 mb-2">
                Diversity of Accepted Suggestions
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-gray-700">
                <div className="bg-gray-50 px-2 py-2 rounded">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-gray-500">
                      {feedbackAnalytics.uniqueGenres} unique
                    </span>
                  </div>
                  <div className="text-gray-800 mt-1">
                    Top genre share:{" "}
                    {(feedbackAnalytics.topGenreShare * 100).toFixed(0)}%
                  </div>
                </div>
                <div className="bg-gray-50 px-2 py-2 rounded">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-gray-500">
                      {feedbackAnalytics.uniqueDirectors} unique
                    </span>
                  </div>
                  <div className="text-gray-800 mt-1">
                    Actors: {feedbackAnalytics.uniqueActors} | Keywords:{" "}
                    {feedbackAnalytics.uniqueKeywords}
                  </div>
                </div>
                <div className="bg-gray-50 px-2 py-2 rounded">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-gray-500">
                      {feedbackAnalytics.positiveCount} total
                    </span>
                  </div>
                  <div className="text-gray-800 mt-1">
                    Higher spread = better novelty balance
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border rounded p-3 mt-3">
              ...
              <div className="text-xs text-gray-700">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="font-medium text-gray-800">
                    {regretStats.regretCount}/{regretStats.totalCandidates}
                  </span>
                </div>
                {regretStats.examples.length > 0 && (
                  <div className="text-gray-600">
                    Examples: {regretStats.examples.join(", ")}
                  </div>
                )}
              </div>
            </div>
          ...
          {sourceConsensus.length > 0 && (
            <div className="bg-white border rounded p-3 mt-3">
              <div className="text-sm font-medium text-gray-900 mb-2">
                Per-Source by Consensus
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-xs text-gray-700">
                {sourceConsensus.map((entry) => {
                  const highRate =
                    entry.high.total > 0
                      ? entry.high.pos / entry.high.total
                      : null;
                  const medRate =
                    entry.medium.total > 0
                      ? entry.medium.pos / entry.medium.total
                      : null;
                  const lowRate =
                    entry.low.total > 0
                      ? entry.low.pos / entry.low.total
                      : null;
                  return (
                    <div
                      key={entry.source}
                      className="bg-gray-50 px-2 py-2 rounded space-y-1"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-gray-800">
                          {entry.source}
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-800">
                          {highRate != null
                            ? `${(highRate * 100).toFixed(0)}% (${entry.high.pos}/${entry.high.total})`
                            : "—"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-800">
                          {medRate != null
                            ? `${(medRate * 100).toFixed(0)}% (${entry.medium.pos}/${entry.medium.total})`
                            : "—"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-800">
                          {lowRate != null
                            ? `${(lowRate * 100).toFixed(0)}% (${entry.low.pos}/${entry.low.total})`
                            : "—"}
                        </span>
                      </div>
                    </div>
      ...
      {stats &&
        ...
        (stats.watchlistTopGenres?.length > 0 ||
          stats.watchlistTopDirectors?.length > 0) && (
          <div className="bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-cyan-950/30 dark:to-blue-950/30 border border-cyan-200 dark:border-cyan-800 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-cyan-700 dark:text-cyan-300 bg-cyan-100 dark:bg-cyan-900/40 px-2 py-1 rounded">
                {stats.watchlistCount} films on watchlist
              </span>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Your watchlist shows what you&apos;re interested in — this signals
              positive intent to the recommendation algorithm.
            </p>

            {/* Avoidance Overrides - items on watchlist that would otherwise be avoided */}
            {stats.avoidanceOverrides &&
              (stats.avoidanceOverrides.genres.length > 0 ||
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                  ...
                  <p className="text-xs text-green-700 mb-3">
                    These would be avoided based on ratings, but your watchlist
                    shows interest — they won&apos;t be filtered out:
                  </p>
                  ...
                  {stats.avoidanceOverrides.genres.length > 0 && (
                    <div className="mb-2">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {stats.avoidanceOverrides.genres.map(
                          ({ name, watchlistCount }) => (
                            <span
                              key={name}
                              className="px-2 py-0.5 rounded-full text-xs bg-green-200 text-green-800"
                            >
                              {name}{" "}
                  ...
                  {stats.avoidanceOverrides.directors.length > 0 && (
                    <div className="mb-2">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {stats.avoidanceOverrides.directors.map(
                          ({ name, watchlistCount }) => (
                            <span
                              key={name}
                              className="px-2 py-0.5 rounded-full text-xs bg-blue-200 text-blue-800"
                            >
                              {name}{" "}
                  ...
                  {stats.avoidanceOverrides.keywords.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {stats.avoidanceOverrides.keywords
                          .slice(0, 8)
                          .map(({ name, watchlistCount }) => (
                            <span
                              key={name}
                              className="px-2 py-0.5 rounded-full text-xs bg-emerald-200 text-emerald-800"
                            >
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {stats.watchlistWithDatesCount > 0 && (
                <div className="bg-white rounded-lg p-4 border border-cyan-100">
                  ...
                  <p className="text-xs text-gray-600 mb-3">
                    Recency-weighted intent (newer entries boost suggestions
                    more)
                  </p>
                  <div className="flex flex-wrap gap-1 mb-2">
                    <span className="px-2 py-1 rounded text-xs bg-emerald-100 text-emerald-700">
                      Fresh ≤90d: {stats.watchlistRecencyBuckets.fresh}
                    </span>
                    <span className="px-2 py-1 rounded text-xs bg-blue-100 text-blue-700">
                      Warm 91-180d: {stats.watchlistRecencyBuckets.warm}
                    </span>
                    <span className="px-2 py-1 rounded text-xs bg-indigo-100 text-indigo-700">
                      Cooling 181-365d: {stats.watchlistRecencyBuckets.cool}
                    </span>
                    <span className="px-2 py-1 rounded text-xs bg-gray-100 text-gray-700">
                      Stale 366d+: {stats.watchlistRecencyBuckets.stale}
                    </span>
                  </div>
                  {(stats.medianWatchlistAge != null ||
                    ...
                    <div className="text-xs text-gray-700 space-y-1">
                      {stats.medianWatchlistAge != null && (
                        <div>
                          Median age: {Math.round(stats.medianWatchlistAge)}{" "}
                          days
                        </div>
                      )}
                      {stats.avgWatchlistAge != null && (
                        <div>
                          Average age: {Math.round(stats.avgWatchlistAge)} days
                        </div>
                      )}
                      <div className="text-gray-500">
                        {stats.watchlistWithDatesCount} with saved add dates
                      </div>
                    </div>
                  )}
                </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {stats.watchlistTopGenres
                        ...
                        .map(({ name, count }) => (
                          <span
                            key={name}
                            className="px-2 py-1 rounded text-xs bg-cyan-100 text-cyan-700"
                          >
                            {name} ({count})
                          </span>
                        ))}
              ...
              {/* Watchlist Actors */}
              {stats.watchlistTopActors &&
                stats.watchlistTopActors.length > 0 && (
                  <div className="bg-white rounded-lg p-4 border border-cyan-100">
                    <h3 className="font-medium text-gray-900 mb-2 text-sm">
                      Actors You Want
                    </h3>
              ...
              {/* Watchlist Keywords */}
              {stats.watchlistTopKeywords &&
                stats.watchlistTopKeywords.length > 0 && (
                  <div className="bg-white rounded-lg p-4 border border-cyan-100">
                    <h3 className="font-medium text-gray-900 mb-2 text-sm">
                      Themes You Want
                    </h3>
            ...
            <p className="text-xs text-gray-500 mt-4">
              💡 Your watchlist helps discover what you want — these patterns
              boost matching recommendations and can override negative signals.
            </p>
          </div>
        )}
      ...
      {stats &&
        !loadingDetails &&
        (stats.avoidedGenres?.length > 0 ||
          stats.avoidedKeywords?.length > 0 ||
          stats.avoidedDirectors?.length > 0 ||
          stats.mixedGenres?.length > 0 ||
          stats.mixedKeywords?.length > 0 ||
          stats.mixedDirectors?.length > 0) && (
          <div className="bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/30 dark:to-orange-950/30 border border-red-200 dark:border-red-800 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/40 px-2 py-1 rounded">
                Filters Suggestions
              </span>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Comparing {stats.likedFilmsCount} liked films (3+ stars or ❤️) vs{" "}
              {stats.dislikedFilmsCount} disliked films (≤1.5 stars).
              <strong>
                {" "}
                Only avoided if you dislike 60%+ of films with that attribute.
              </strong>
              <br />
              <span className="text-xs text-gray-500">
                Note: Films rated 2-2.5 stars are &quot;meh&quot; (neutral) and
                don&apos;t count as dislikes. Unrated films are also neutral.
              </span>
            </p>
            ...
            {((stats.mixedGenres && stats.mixedGenres.length > 0) ||
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                ...
                <p className="text-xs text-green-700 mb-3">
                  You&apos;ve disliked some films with these, but you&apos;ve
                  liked MORE — so they&apos;re not avoided:
                </p>
            ...
            <p className="text-xs text-gray-500 mt-4">
              💡 &quot;Disliked&quot; = rated ≤1.5 stars. Films rated 2+ stars
              are not considered dislikes. &quot;Guilty pleasures&quot;
              (low-rated but ❤️ liked) don&apos;t count as dislikes either.
        ...
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 border border-indigo-200 dark:border-indigo-800 rounded-lg p-4 mb-6">
          ...
          <div className="space-y-3">
            <div className="bg-white rounded p-3">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${explorationStats.exploration_rate * 3.33 * 100}%`,
                  }}
                />
              </div>
              <div className="text-xs text-gray-600 mt-2">
                {explorationStats.exploration_rate !== 0.15 ? (
                  explorationStats.exploration_rate > 0.15 ? (
                    <span className="text-green-700">
                      ✓ Increased because you enjoy exploratory picks (avg{" "}
                      {explorationStats.exploratory_avg_rating.toFixed(1)}★)
                    </span>
                  ) : (
                    <span className="text-orange-700">
                      ↓ Decreased to focus on safer recommendations (avg{" "}
                      {explorationStats.exploratory_avg_rating.toFixed(1)}★)
                    </span>
                  )
                ) : (
                  <span>
                    Default rate • Will adjust based on your ratings (
                    {explorationStats.exploratory_films_rated} exploratory films
                    rated so far)
                  </span>
                )}
              </div>
            ...
            <div className="text-xs text-gray-600 bg-white rounded p-2">
              ...
              {(explorationStats.exploration_rate * 100).toFixed(0)}% of your
              suggestions will be &quot;discovery picks&quot; from adjacent
              genres or acclaimed films outside your usual taste. The other{" "}
              {(100 - explorationStats.exploration_rate * 100).toFixed(0)}% are
                  ...
                  {adjacentPrefs.slice(0, 5).map((pref, idx) => (
                    ...
                    >
                      ...
                      <span className="text-green-700 font-medium">
                        {(pref.success_rate * 100).toFixed(0)}% success (
                        {pref.rating_count} films)
                      </span>
                    </div>
                  ))}
                </div>
      ...
      {pairwiseStats && pairwiseStats.total_comparisons > 0 && (
        <div className="bg-gradient-to-r from-violet-50 to-fuchsia-50 dark:from-violet-950/30 dark:to-fuchsia-950/30 border border-violet-200 dark:border-violet-800 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            When you choose between two similar films, the algorithm learns your
            subtle preferences. These comparisons help refine recommendations by
            understanding which features matter most to you.
          </p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded p-3 border border-violet-100 dark:border-violet-900">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
              <div>
                • <strong>Consensus level</strong> shows how many sources agreed
                on each film (high = 3+ sources, medium = 2, low = 1)
              </div>
              <div>
                • When you pick a film, the algorithm learns that its{" "}
                <strong>specific features</strong> (genre, actors, themes, year)
                were more appealing
              </div>
              <div>
                • These micro-preferences accumulate to fine-tune your taste
                profile beyond simple ratings
              </div>
              <div>
                • Choices between high-consensus films teach the algorithm about
                quality thresholds
              </div>
              <div>
                • Choices between low-consensus films reveal your openness to
                niche/exploratory picks
              </div>
            </div>
          </div>

          <div className="text-xs text-gray-500 dark:text-gray-400 bg-violet-50 dark:bg-violet-950/30 rounded p-2 mt-3">
            💡 <strong>Tip:</strong> The more comparisons you make, the better
            the algorithm understands your nuanced preferences. Try the pairwise
            comparison feature on the Suggestions page to help refine your
            recommendations!
          </div>
        </div>
      )}
      ...
      {repeatSuggestionStats && repeatSuggestionStats.totalExposures > 0 && (
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-950/30 dark:to-cyan-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            This tracks how often you see the same suggestions, helping ensure
            fresh recommendations. Lower repeat rates mean you&apos;re getting
            more variety.
          </p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded p-3 border border-blue-100 dark:border-blue-900">
              ...
              <div
                className={`text-2xl font-bold ${
                  repeatSuggestionStats.repeatRate < 0.1
                    ? "text-green-700 dark:text-green-400"
                    : repeatSuggestionStats.repeatRate < 0.2
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {repeatSuggestionStats.avgTimeBetweenRepeats !== null && (
                  <>
                    Avg {repeatSuggestionStats.avgTimeBetweenRepeats.toFixed(1)}{" "}
                    days between
                  </>
                )}
              </div>
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-950/30 rounded p-2 mt-3">
            💡 <strong>Note:</strong> A repeat rate under 15% is ideal. If
            you&apos;re seeing the same suggestions too often, try adjusting the
            Discovery slider or exploring different categories.
          </div>
        </div>
      )}
