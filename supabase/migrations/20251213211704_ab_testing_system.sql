...
drop policy if exists "Anyone can view active AB tests" on ab_test_configs;
...
drop policy if exists "Users can view their own AB test assignments" on ab_test_assignments;
...
drop policy if exists "Users can insert their own AB test metrics" on ab_test_metrics;
create policy "Users can insert their own AB test metrics"
  on ab_test_metrics for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can view their own AB test metrics" on ab_test_metrics;
