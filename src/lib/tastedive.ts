...
import {
  getCachedTasteDiveSimilar,
  setCachedTasteDiveSimilar,
} from "@/lib/apiCache";
...
export async function getSimilarContent(
  ...
  try {
    ...
    try {
      const cached = await getCachedTasteDiveSimilar(cacheKey);
      if (cached) {
        return cached;
      }
    } catch (cacheError) {
      console.warn("[TasteDive] Cache read failed:", cacheError);
    }
    ...
    const results: TasteDiveResult[] = rawResults.map(
    ...
    try {
      await setCachedTasteDiveSimilar(cacheKey, results);
    } catch (cacheError) {
      console.warn("[TasteDive] Cache write failed:", cacheError);
    }

    console.log("[TasteDive] Found results", {
