...
drop policy if exists "Users can read their own pairwise events" on pairwise_events;
create policy "Users can read their own pairwise events"
  on pairwise_events for select
  using (auth.uid() = user_id);

drop policy if exists "Users can insert their own pairwise events" on pairwise_events;
create policy "Users can insert their own pairwise events"
  on pairwise_events for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update their own pairwise events" on pairwise_events;
