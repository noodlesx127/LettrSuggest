  ...
  return (
    <AuthGate>
      ...
      <p className="text-sm text-gray-700 mb-4">
        Films you want to watch. These are not used for suggestions but you can
        browse them here.
      </p>
      {unmappedCount > 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
          <p className="text-sm text-yellow-800">
            <strong>
              {unmappedCount} of {gridFilms.length} watchlist films
            </strong>{" "}
            don&apos;t have poster images yet. Re-import your data to map them
            to TMDB and see their posters.
          </p>
        </div>
      )}
      {loadingFilms && (
        <p className="text-sm text-gray-600">
          Loading your watchlist from database…
        </p>
      )}
  ...
  return (
    <div className="group relative bg-white rounded shadow-sm border overflow-hidden">
      ...
      <div className="p-2">
        ...
        <p className="text-[10px] mt-1 text-blue-700">On Watchlist</p>
      </div>
    </div>
  );
}
