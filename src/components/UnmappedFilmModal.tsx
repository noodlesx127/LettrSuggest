...
import { fetchMovieById, searchTmdb, upsertFilmMapping } from "@/lib/enrich";
...
export default function UnmappedFilmModal({
  isOpen,
  onClose,
  unmappedFilms,
  userId,
  ...
  const [letterboxdSuggestion, setLetterboxdSuggestion] =
    useState<LetterboxdSuggestion | null>(null);
    ...
    const lookupLetterboxd = async () => {
      ...
      try {
        const res = await fetch(
          `/api/letterboxd/tmdb-id?uri=${encodeURIComponent(currentFilm.uri)}`,
        );
  ...
  const applyMapping = async (tmdbId: number, movieData?: any) => {
    ...
    try {
      ...
      if (chosen) {
        ...
        if (error) {
          console.error("[UnmappedModal] Cache upsert failed", {
            tmdbId,
            error,
          });
        }
      ...
      <div className="fixed inset-4 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-2xl md:max-h-[80vh] bg-white dark:bg-gray-800 rounded-lg shadow-xl z-50 flex flex-col overflow-hidden">
        ...
        <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border-b dark:border-gray-700">
          ...
          <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
            This film couldn&apos;t be automatically matched to TMDB. Search
            below to find the correct match.
          </p>
        </div>
            ...
            {letterboxdSuggestion.loading ? (
              ...
              >
                <div className="w-12 h-18 bg-gray-200 dark:bg-gray-700 rounded overflow-hidden flex-shrink-0 relative">
                  {letterboxdSuggestion.movie.poster_path ? (
                    <Image
                      ...
                      alt={
                        letterboxdSuggestion.movie.title ||
                        letterboxdSuggestion.movie.name
                      }
                      fill
                      sizes="48px"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400">
                    {(
                      letterboxdSuggestion.movie.release_date ||
                      letterboxdSuggestion.movie.first_air_date
                    )?.slice(0, 4) || "Unknown year"}
                    {" • "}
                    {letterboxdSuggestion.mediaType === "tv"
                      ? "TV Show"
                      : "Movie"}
                    {" • "}
