...
3. **Recommendations**: Multi-source aggregator (`recommendationAggregator.ts`) combines TMDB, TasteDive, TuiMDB, Watchmode → consensus scoring → filtered suggestions

### Key Components

- **Server Actions** (`src/app/actions/`): Use `'use server'` directive for API-key-protected operations (recommendations, enrichment)
- **API Routes** (`src/app/api/`): Proxy external APIs (TMDB, etc.) with server-side keys. All routes use `fetchWithRetry` with exponential backoff
- **Supabase**: RLS-enabled per-user tables (`film_events`, `film_tmdb_map`, `suggestion_feedback`) + shared caches (`tmdb_movies`, `tmdb_similar_cache`)
...
- **Cache-first**: Check Supabase cache tables (`tmdb_similar_cache`, `tuimdb_uid_cache`) before API calls
...
- **Example**: See `getCachedTMDBSimilar()` / `setCachedTMDBSimilar()` in `apiCache.ts`

### Recommendation System

- **Aggregator** (`recommendationAggregator.ts`): Fetches from 4 sources in parallel, deduplicates by TMDB ID, calculates consensus score
...
```pwsh
npm install
# Set .env.local with NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, TMDB_API_KEY, TUIMDB_API_KEY
...
- **Required Keys**: All `NEXT_PUBLIC_*`, `TMDB_API_KEY`, `TUIMDB_API_KEY`

## Code Conventions

### File Organization

- **Route Handlers**: `src/app/api/[service]/[endpoint]/route.ts` (TMDB, TuiMDB, etc.)
...
- **Functions**: Descriptive verbs (`fetchTmdbMovie`, `upsertFilmMapping`, `getCachedTMDBSimilar`)
- **Types**: PascalCase with descriptive names (`AggregatedRecommendation`, `SourceRecommendation`)
- **Constants**: SCREAMING_SNAKE_CASE for config (`TMDB_CACHE_TTL_DAYS`)
