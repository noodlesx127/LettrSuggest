...
export default function UserQuiz({ userId, isOpen, onClose }: UserQuizProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div
        className={`bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full overflow-hidden transition-all ${currentQuestion?.type === "movie_rating" || currentQuestion?.type === "pairwise" ? "max-w-2xl" : "max-w-lg"}`}
      >
        {/* Header */}
        ...
        <div className="p-6 min-h-[300px] flex flex-col">
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                You&apos;ve answered all available quiz questions. Check back
                later for more!
              </p>
              <button
                onClick={onClose}
        ...
        {stats && !loading && (
          <div className="px-6 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
            <p className="text-xs text-center text-gray-500 dark:text-gray-400">
              📊 You&apos;ve answered {stats.totalAnswered} quiz question
              {stats.totalAnswered !== 1 ? "s" : ""} total
            </p>
          </div>
        )}
      </div>
...
function CompletedView({
  ...
  stats: { totalAnswered: number } | null;
  onClose: () => void;
}) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 max-w-sm">
        Thanks for helping us understand your taste better. Your suggestions
        will now be more personalized!
      </p>
      {stats && (
          ...
          <button
            ...
            onClick={() =>
              onAnswer({
                ...
                loserId:
                  idx === 0 ? question.movieB.tmdbId : question.movieA.tmdbId,
              })
            }
            disabled={submitting}
