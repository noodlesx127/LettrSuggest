...
export async function searchMovies(
  ...
  if (preferTuiMDB) {
    try {
      ...
      if (tuiR.ok && tuiJ.ok && tuiJ.results && tuiJ.results.length > 0) {
        console.log("[UnifiedAPI] TuiMDB search successful", {
          count: tuiJ.results.length,
        });
        ...
        try {
          ...
          if (
            ...
            tmdbJ.ok &&
            tmdbJ.results &&
            tmdbJ.results.length > 0
          ) {
            // Merge TuiMDB UIDs into TMDB results by matching titles
            const tuiResults = tuiJ.results as Array<{
              UID: number;
              Title: string;
              ReleaseDate?: string;
            }>;
            ...
            for (const tmdbMovie of tmdbResults) {
              ...
              for (const tuiMovie of tuiResults) {
                ...
                const tuiYear = tuiMovie.ReleaseDate
                  ? new Date(tuiMovie.ReleaseDate).getFullYear()
                  : null;

                // Match if titles are the same and years match (if both exist)
                if (
                  tmdbTitle === tuiTitle &&
                  (!tmdbYear || !tuiYear || tmdbYear === tuiYear)
                ) {
          ...
          console.error(
            "[UnifiedAPI] TMDB search failed during TuiMDB merge",
            tmdbError,
          );
        }
        ...
        console.log(
          "[UnifiedAPI] Could not get TMDB IDs, falling through to TMDB-only search",
        );
      } else {
      ...
      console.error(
        "[UnifiedAPI] TuiMDB search failed, falling back to TMDB",
        error,
      );
    }
  }
  ...
  if (!r.ok || !j.ok) {
    console.error("[UnifiedAPI] TMDB search error", {
      status: r.status,
      body: j,
    });
...
export async function getMovieDetails(
  ...
  if (preferTuiMDB) {
    try {
      ...
      u.searchParams.set("tmdb_id", String(id));
      u.searchParams.set("_t", String(Date.now())); // Cache buster
      ...
      if (r.ok && j.ok && j.movie) {
        console.log("[UnifiedAPI] TuiMDB fetch successful", {
          id,
          title: j.movie.title,
        });
        return j.movie;
      ...
      console.error(
        "[UnifiedAPI] TuiMDB fetch failed, falling back to TMDB",
        error,
      );
    }
  }
  ...
  try {
    ...
    if (!r.ok || !j.ok) {
      console.error("[UnifiedAPI] TMDB fetch error", {
        status: r.status,
        body: j,
      });
      return null;
