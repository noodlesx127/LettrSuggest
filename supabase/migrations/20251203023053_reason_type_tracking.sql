...
drop policy if exists "Users can read their own reason preferences" on user_reason_preferences;
create policy "Users can read their own reason preferences"
  on user_reason_preferences for select
  using (auth.uid() = user_id);

drop policy if exists "Users can update their own reason preferences" on user_reason_preferences;
create policy "Users can update their own reason preferences"
  on user_reason_preferences for update
  using (auth.uid() = user_id);

drop policy if exists "Users can insert their own reason preferences" on user_reason_preferences;
