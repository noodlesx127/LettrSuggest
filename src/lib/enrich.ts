...
export type TMDBMovie = {
  ...
  enhanced_genres?: Array<{
    id: number;
    name: string;
    source: "tmdb" | "tuimdb";
  }>; // Merged TMDB + TuiMDB genres
...
export async function upsertFilmMapping(
  ...
  const { error } = await supabase
    .from("film_tmdb_map")
    .upsert(
      { user_id: userId, uri, tmdb_id: tmdbId },
      { onConflict: "user_id,uri" },
    );
  if (error) {
    console.error("[Supabase] upsertFilmMapping error", {
      userId,
      uri,
      tmdbId,
      error,
 ...
 */
export async function getBulkTmdbDetails(
  tmdbIds: number[],
  ...
  try {
    ...
    for (let i = 0; i < tmdbIds.length; i += chunkSize) {
      ...
      if (error) {
        console.error("[BulkTmdb] Error fetching chunk", {
          error,
          chunkStart: i,
        });
        continue;
...
export async function blockSuggestion(userId: string, tmdbId: number) {
  ...
  if (error && error.code !== "23505") {
    ...
    console.error("[Supabase] blockSuggestion error", {
      userId,
      tmdbId,
      error,
    });
...
async function clearSuggestionFeedback(
  userId: string,
  tmdbId: number,
): Promise<void> {
  if (!supabase) throw new Error("Supabase not initialized");
  ...
  if (error) {
    console.error("[Supabase] clearSuggestionFeedback fetch error", {
      userId,
      tmdbId,
      error,
    });
  ...
  if (!features) {
    try {
      ...
      console.error(
        "[Supabase] clearSuggestionFeedback extract features failed",
        { tmdbId, error: e },
      );
    }
  }
  ...
  if (deleteError) {
    console.error("[Supabase] clearSuggestionFeedback delete error", {
      userId,
      tmdbId,
      deleteError,
    });
...
export async function unblockSuggestion(
  userId: string,
  tmdbId: number,
  opts?: { skipFeedbackClear?: boolean },
) {
  if (!supabase) throw new Error("Supabase not initialized");
  ...
  const { error } = await supabase
    ...
    .eq("tmdb_id", tmdbId);
  if (error) {
    console.error("[Supabase] unblockSuggestion error", {
      userId,
      tmdbId,
      error,
    });
...
export async function removeAllowedSuggestion(
  userId: string,
  tmdbId: number,
): Promise<void> {
  try {
...
export async function neutralizeFeedback(
  userId: string,
  tmdbId: number,
): Promise<void> {
  if (!supabase) throw new Error("Supabase not initialized");
...
export async function getAllowedSuggestions(
  userId: string,
): Promise<Set<number>> {
  if (!supabase) throw new Error("Supabase not initialized");
  const { data, error } = await supabase
...
function extractReasonTypes(reasons: string[]): string[] {
  ...
  for (const reason of reasons) {
    ...
    if (
      ...
      lower.includes("sequel") ||
      lower.includes("prequel")
    ) {
    ...
    if (
      ...
      (lower.includes("watch") || lower.includes("favorite"))
    ) {
      types.add("recent");
    }
  }
...
export async function addFeedback(
  ...
  try {
    ...
    console.error("[FeatureFeedback] Failed to extract movie features", {
      tmdbId,
      error: e,
    });
  }
  ...
  const existingFeatures = await getExistingFeaturePreferences(
    userId,
    movieFeatures,
  );
  ...
  if (error) {
    console.error("[Supabase] addFeedback error", {
      userId,
      tmdbId,
      type,
      error,
...
async function applyFeatureDelta(
  ...
  try {
    ...
    const positiveCount =
      (existing?.positive_count || 0) + update.deltaPositive;
    const negativeCount =
      (existing?.negative_count || 0) + update.deltaNegative;
    ...
    console.error("[PairwiseLearning] Failed to apply feature delta", {
      update,
      error: e,
    });
  }
...
export async function applyPairwiseFeatureLearning(
  userId: string,
  winnerTmdbId: number,
  loserTmdbId: number,
): Promise<void> {
  ...
  try {
    ...
    const winnerKeys = new Set(
      winnerSlice.map((f) => `${f.feature_type}:${f.feature_id}`),
    );
    const loserKeys = new Set(
      loserSlice.map((f) => `${f.feature_type}:${f.feature_id}`),
    );
 ...
 */
const sourceReliabilityCache = new Map<
  string,
...
export async function fetchSourceReliability(
  ...
  try {
    ...
    data?.forEach((row: any) => {
      const sources: string[] = Array.isArray(row.recommendation_sources)
        ? row.recommendation_sources
        : [];
      const isPos = row.feedback_type === "positive";
      const level =
        (row.consensus_level as "high" | "medium" | "low" | null) ?? "low";
...
export async function getFeatureEvidenceSummary(
  ...
  for (const [type, names] of grouped.entries()) {
    ...
    try {
      ...
      if (error) {
        console.error("[FeatureEvidence] Failed to fetch evidence", {
          type,
          error,
        });
        continue;
      ...
      console.error("[FeatureEvidence] Exception while fetching evidence", {
        type,
        error: e,
      });
    }
...
async function updateFeaturePreferences(
  ...
  for (const update of updates) {
    try {
      ...
      const positiveCount =
        (existing?.positive_count || 0) + (isPositive ? 1 : 0);
      const negativeCount =
        (existing?.negative_count || 0) + (isPositive ? 0 : 1);
      const total = positiveCount + negativeCount;
...
async function detectFeedbackPatterns(userId: string): Promise<string[]> {
  ...
  try {
    ...
    const { data: avoidedFeatures } = await supabase
      .from("user_feature_feedback")
      .select(
        "feature_type, feature_id, feature_name, negative_count, positive_count, inferred_preference",
      )
      .eq("user_id", userId)
    ...
    const avoidedActors = avoidedFeatures.filter(
      (f) =>
        f.feature_type === "actor" &&
        f.negative_count >= 3 &&
        f.inferred_preference <= 0.3,
    );
    ...
    const avoidedKeywords = avoidedFeatures.filter(
      (f) =>
        f.feature_type === "keyword" &&
        f.negative_count >= 3 &&
        f.inferred_preference <= 0.25,
    );
...
export async function getAvoidedFeatures(userId: string): Promise<{
  ...
  avoidFranchises: Array<{
    id: number;
    name: string;
    weight: number;
    count: number;
  }>;
  avoidDirectors: Array<{
    id: number;
    name: string;
    weight: number;
    count: number;
  ...
  preferKeywords: Array<{
    id: number;
    name: string;
    weight: number;
    count: number;
  }>;
  preferDirectors: Array<{
    id: number;
    name: string;
    weight: number;
    count: number;
  ...
  try {
    ...
    const { data } = await supabase
      .from("user_feature_feedback")
      .select(
        "feature_type, feature_id, feature_name, negative_count, positive_count, inferred_preference, last_updated",
      )
      .eq("user_id", userId)
    ...
    const avoidActors = data
      .filter(
        (f) =>
          f.feature_type === "actor" &&
          f.negative_count - f.positive_count >= 1,
      )
      .map((f) => ({
        id: f.feature_id,
        name: f.feature_name,
        weight: applyRecencyDecay(
          calcGraduatedPenalty(f.negative_count, f.positive_count),
          f.last_updated,
        ),
    ...
    const avoidFranchises = data
      ...
      .map((f) => ({
        ...
        weight:
          f.negative_count === 1
            ? 1.5
            : Math.min(2.5 + f.negative_count * 0.5, 4.0),
        count: f.negative_count,
      }))
      .sort((a, b) => b.weight - a.weight)
      .slice(0, 10);
    ...
    const preferActors = data
      .filter(
        (f) =>
          f.feature_type === "actor" &&
          f.positive_count - f.negative_count >= 1,
      )
      .map((f) => ({
        id: f.feature_id,
        name: f.feature_name,
        weight: applyRecencyDecay(
          calcGraduatedBoost(f.negative_count, f.positive_count),
          f.last_updated,
        ),
    ...
    const avoidGenres = data
      .filter(
        (f) =>
          f.feature_type === "genre" &&
          f.negative_count - f.positive_count >= 2,
      )
      .map((f) => ({
        id: f.feature_id,
        name: f.feature_name,
    ...
    const preferGenres = data
      .filter(
        (f) =>
          f.feature_type === "genre" &&
          f.positive_count - f.negative_count >= 2,
      )
      .map((f) => ({
        id: f.feature_id,
        name: f.feature_name,
 ...
 */
async function updateReasonPreferences(
  userId: string,
  reasonTypes: string[],
  isPositive: boolean,
) {
  ...
  try {
    for (const reasonType of reasonTypes) {
      ...
      const successCount =
        (existing?.success_count || 0) + (isPositive ? 1 : 0);
      const totalCount = (existing?.total_count || 0) + 1;
      const successRate = totalCount > 0 ? successCount / totalCount : 0.5;

      const { error } = await supabase.from("user_reason_preferences").upsert(
        {
          user_id: userId,
          reason_type: reasonType,
          success_count: successCount,
      ...
      if (error) {
        console.error("[AdaptiveLearning] Error updating reason preference", {
          reasonType,
          error,
        });
      }
 ...
 */
export async function getReasonPreferences(
  userId: string,
): Promise<Map<string, number>> {
  ...
  try {
    ...
    if (error) {
      console.error(
        "[AdaptiveLearning] Error fetching reason preferences",
        error,
      );
      return new Map();
    }
...
export async function getFeedback(
  userId: string,
): Promise<Map<number, "negative" | "positive">> {
  if (!supabase) throw new Error("Supabase not initialized");
  const { data, error } = await supabase
    .from("suggestion_feedback")
...
export async function fetchTmdbMovie(id: number): Promise<TMDBMovie | null> {
  ...
  try {
    ...
    const j = await r.json();
    if (!r.ok || !j.ok) {
      console.error("[UnifiedAPI] TMDB fetch movie error", {
        id,
        status: r.status,
        body: j,
      });
...
function extractFeatures(movie: TMDBMovie) {
  ...
  const genreSource =
    (movie as any).enhanced_genres || (movie as any).genres || [];
  const genres: string[] = Array.isArray(genreSource)
    ? genreSource.map((g: any) => g.name).filter(Boolean)
    : [];
  const genreIds: number[] = Array.isArray(genreSource)
    ? genreSource.map((g: any) => g.id).filter(Boolean)
    : [];
  const genreSources: string[] = Array.isArray(genreSource)
    ? genreSource.map((g: any) => g.source || "tmdb")
    : [];

  // Check for seasonal genres from TuiMDB (e.g., Christmas, Halloween)
  const seasonalInfo = getCurrentSeasonalGenres();
  const hasSeasonalGenre = genreIds.some((id) =>
    seasonalInfo.genres.includes(id),
  );
  ...
  const productionCompanyIds = (movie.production_companies || []).map(
    (c) => c.id,
  );
  ...
  const isChildrens = keywords.some(
    (k) =>
...
export async function fetchTmdbMovieCached(
  id: number,
): Promise<TMDBMovie | null> {
  try {
    if (!supabase) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        "[Enrich] API route failed, falling back to direct TMDB fetch",
      );
      const fresh = await withTimeout(fetchTmdbMovie(id));
      return fresh;
    }
    ...
    console.warn(
      "[Enrich] API route error, falling back to direct TMDB fetch:",
      apiError,
    );
    const fresh = await withTimeout(fetchTmdbMovie(id));
    return fresh;
...
export async function buildTasteProfile(params: {
  ...
  const rewatchRate =
    params.films.length > 0 ? rewatchCount / params.films.length : 0;
  ...
  const dislikedFilms = params.films.filter(
    (f) =>
      hasRealRating(f.rating) &&
      f.rating! <= DISLIKE_THRESHOLD &&
      params.mappings.has(f.uri) &&
      !f.liked,
  );

  // Count "guilty pleasures" - low-rated but liked films (these are NOT treated as dislikes)
  const guiltyPleasures = params.films.filter(
    (f) =>
      f.liked &&
      hasRealRating(f.rating) &&
      f.rating! <= DISLIKE_THRESHOLD &&
  ...
  const neutralFilms = params.films.filter(
    (f) =>
      params.mappings.has(f.uri) &&
      !f.liked &&
      (!hasRealRating(f.rating) ||
  ...
  const avoidKeywordWeights = new Map<
    number,
    { name: string; weight: number }
  >();
  const avoidDirectorWeights = new Map<
    number,
    { name: string; weight: number }
  >();
  ...
  // Pair film data with movies BEFORE iterating to ensure correct alignment
  const pairedLiked = likedFilms
    .map((film, i) => ({
      movie: likedMovies[i],
      film,
    }))
    .filter(
      (
        ...
        movie: NonNullable<typeof pair.movie>;
        film: typeof pair.film;
      } => pair.movie != null,
    );

  for (const { movie, film } of pairedLiked) {
    ...
    feats.genreIds.forEach((id, idx) => {
      ...
      genreWeights.set(id, {
        name,
        weight: current.weight + weight * genreFraction,
    ...
    const keywordFraction =
      keywordCount > 0 ? 1 / Math.sqrt(Math.min(keywordCount, 10)) : 1;
    feats.keywordIds.forEach((id, idx) => {
      const name = feats.keywords[idx];
      const current = keywordWeights.get(id) || { name, weight: 0, count: 0 };
      keywordWeights.set(id, {
        name,
        weight: current.weight + weight * keywordFraction,
        count: current.count + 1,
      });
      // Track count for ratio-based avoidance
    ...
    castData.slice(0, 5).forEach((actor: any, idx: number) => {
      ...
      const current = actorWeights.get(actor.id) || {
        name: actor.name,
        weight: 0,
        count: 0,
      };
  ...
  // Pair film data with movies BEFORE iterating to ensure correct alignment
  const pairedDisliked = dislikedFilms
    .map((film, i) => ({
      movie: dislikedMovies[i],
      film,
    }))
    .filter(
      (
  ...
  for (const { movie, film } of pairedDisliked) {
    ...
    feats.directorIds.forEach((id, idx) => {
      ...
      avoidDirectorWeights.set(id, {
        name,
        weight: current.weight + negWeight,
      });
    });
  }
  ...
  if (params.watchlistFilms && params.watchlistFilms.length > 0) {
    ...
    const watchlistEntries = params.watchlistFilms
      ...
      .filter(
        (row): row is { id: number; addedAt: string | null } =>
    ...
    for (let i = 0; i < watchlistMovies.length; i++) {
      ...
      feats.genreIds.forEach((id, idx) => {
        ...
        if (name)
          watchlistGenreCounts.set(
            name,
            (watchlistGenreCounts.get(name) || 0) + 1,
          );
        const current = genreWeights.get(id) || { name, weight: 0, count: 0 };
        genreWeights.set(id, {
          name,
          weight: current.weight + totalBoost,
          count: current.count + 1,
        });
      ...
      feats.keywordIds.forEach((id, idx) => {
        ...
        if (name)
          watchlistKeywordCounts.set(
            name,
            (watchlistKeywordCounts.get(name) || 0) + 1,
          );
        const current = keywordWeights.get(id) || { name, weight: 0, count: 0 };
        keywordWeights.set(id, {
          name,
      ...
      feats.directorIds.forEach((id, idx) => {
        ...
        if (name)
          watchlistDirectorCounts.set(
            name,
            (watchlistDirectorCounts.get(name) || 0) + 1,
          );
        const current = directorWeights.get(id) || {
          name,
          weight: 0,
          count: 0,
        };
  ...
  const topDirectors = Array.from(directorWeights.entries())
    ...
    .map(([id, { name, weight, count }]) => ({
      ...
      weight,
      count,
      profile: directorProfiles.get(id),
    }));
  ...
  const mixedGenres: Array<{
    id: number;
    name: string;
    liked: number;
    disliked: number;
  }> = [];
  const mixedKeywords: Array<{
    id: number;
    name: string;
    liked: number;
    disliked: number;
  }> = [];
  const mixedDirectors: Array<{
    id: number;
    name: string;
    liked: number;
    disliked: number;
  ...
  const avoidGenres = Array.from(avoidGenreWeights.entries())
    .filter(([id, { name }]) => {
      ...
      if (
        disliked >= MIN_DISLIKED_FOR_AVOIDANCE &&
        !shouldAvoid(liked, disliked, MIN_DISLIKED_FOR_AVOIDANCE)
      ) {
  ...
  const nicheFilmsForDetection = likedFilms.map((f) => {
    ...
    return {
      title:
        details?.title ||
        f.uri.split("/").filter(Boolean).pop()?.replace(/-/g, " ") ||
        "",
  ...
  try {
    const subgenreFilms = likedFilms.map((f) => {
      ...
      return {
        ...
        genres: details?.genres?.map((g: any) => g.name) || [],
        keywords:
          details?.keywords?.keywords?.map((k: any) => k.name) ||
    ...
    preferredSubgenreKeywordIds =
      getPreferredSubgenreKeywordIds(subgenrePatterns);
    ...
    if (params.userId && supabase) {
      try {
        const { data: explicitPrefs } = await supabase
          .from("user_feature_feedback")
          .select(
            "feature_type, feature_id, feature_name, inferred_preference, positive_count, negative_count",
          )
        ...
        console.error(
          "[TasteProfile] Failed to fetch explicit preferences",
          err,
        );
      }
    }
  ...
  return {
    ...
    avoidKeywords,
    avoidDirectors,
    watchlistGenres: Array.from(watchlistGenreCounts.entries())
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count),
    watchlistKeywords: Array.from(watchlistKeywordCounts.entries())
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count),
    watchlistDirectors: Array.from(watchlistDirectorCounts.entries())
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => b.count - a.count),
    userStats,
    nichePreferences,
    ...
    // Visualization Data
    tasteBins: {
      absoluteFavorites: ratings.filter(
        (r) => r >= 4.5 && params.films.find((f) => f.rating === r)?.liked,
      ).length, // approx
...
>(
  ...
  console.log("[DiversityFilter] Applied diversity filtering", {
    ...
    directorCounts: Array.from(directorCounts.entries())
    ...
    for (let i = 0; i < pool.length; i++) {
      ...
      if (selected.length > 0) {
        diversityPenalty = Math.max(
          ...selected.map((s) => similarity(s, cand)),
        );
      }
...
> {
  ...
  const applyContextBias = (
    ...
    const runtime =
      feats.runtime ?? ((movie as any).runtime as number | undefined);
    const rating = (movie as any).rated as string | undefined;
    const genresLower = feats.genres.map((g) => g.toLowerCase());
    const hasGenre = (needle: string) =>
      genresLower.some((g) => g.includes(needle));
    ...
    switch (context.mode) {
      case "family": {
        ...
        if (isHorror || isThriller || isCrime)
          return { delta: 0, reasons, filtered: true };
        if (ratingUpper.startsWith("R") || ratingUpper.includes("NC-17"))
          return { delta: 0, reasons, filtered: true };
      ...
      case "immersive": {
        if (runtime && runtime >= 130) {
          delta += 0.9;
          reasons.push(
            `Longer runtime suits an immersive session (~${runtime}m)`,
          );
        }
      ...
      case "background": {
        if (runtime) {
          if (runtime >= 80 && runtime <= 115) {
            delta += 0.5;
            reasons.push(
              `Manageable runtime for background viewing (~${runtime}m)`,
            );
          } else if (runtime >= 145) {
            delta -= 0.8;
  ...
  const watchedNotLiked = params.films.filter(
    (f) =>
  ...
  const buildSourceReliability = () => {
    ...
    for (const [tmdbId, type] of feedbackMap.entries()) {
      ...
      if (!meta) continue;
      const keyType =
        type === "positive" ? "pos" : type === "negative" ? "neg" : null;
      if (!keyType) continue;
  ...
  const userSourceReliability =
    params.sourceReliability ?? buildSourceReliability();
  ...
  const watchlistIntentMap = new Map<
    number,
    { boost: number; label: string; ageText: string }
  >();
  if (params.watchlistEntries && params.watchlistEntries.length > 0) {
    ...
    const computeRecency = (addedAt?: string | null) => {
      if (!addedAt) {
        return {
          boost: 0.6,
          label: "saved in your watchlist",
          ageText: "saved previously",
        };
      }

      const days = Math.max(
        1,
        Math.round((now - new Date(addedAt).getTime()) / (1000 * 60 * 60 * 24)),
      );

      if (days <= 30)
        return { boost: 1.2, label: "recently added", ageText: `${days}d ago` };
      if (days <= 90)
        return {
          boost: 1.0,
  ...
  const watchlistTmdbIds = new Set(
    params.watchlistEntries?.map((e) => e.tmdbId) ?? [],
  );
  const candidatesAfterWatchlistExclusion = validCandidates.filter(
    (id) => !watchlistTmdbIds.has(id),
  );

  if (watchlistTmdbIds.size > 0) {
    console.log(
      "[SuggestByOverlap] Excluded watchlist movies from main suggestions:",
      {
  ...
  const dislikedMovies = await mapLimit(dislikedIds, 10, async (id) => {
    try {
      ...
      console.error(
        `[SuggestByOverlap] Failed to fetch disliked movie ${id}`,
        e,
      );
      return null;
    }
  ...
  for (let i = 0; i < likedFeats.length; i++) {
    ...
    for (const g of f.genres)
      pref.genres.set(g, (pref.genres.get(g) ?? 0) + weight);
    if (f.genreCombo)
      pref.genreCombos.set(
        f.genreCombo,
        (pref.genreCombos.get(f.genreCombo) ?? 0) + weight,
      );
    ...
    for (const d of f.directors) {
      ...
      if (!pref.directorKeywords.has(d))
        pref.directorKeywords.set(d, new Set());
    ...
    for (const studio of f.productionCompanies) {
      pref.productionCompanies.set(
        studio,
        (pref.productionCompanies.get(studio) ?? 0) + weight,
      );
    }

    for (const k of f.keywords)
      pref.keywords.set(k, (pref.keywords.get(k) ?? 0) + weight);
  ...
  for (const f of dislikedFeats) {
    if (f.genreCombo)
      avoid.genreCombos.set(
        f.genreCombo,
        (avoid.genreCombos.get(f.genreCombo) ?? 0) + 1,
      );
    for (const k of f.keywords)
      avoid.keywords.set(k, (avoid.keywords.get(k) ?? 0) + 1);
  }
  ...
  const avoidsAnimation =
    totalLiked > 10 && likedAnimationCount / totalLiked < animationThreshold;
  const avoidsFamily =
    totalLiked > 10 && likedFamilyCount / totalLiked < animationThreshold;
  const avoidsChildrens =
    totalLiked > 10 && likedChildrensCount / totalLiked < animationThreshold;
  ...
  console.log("[Suggest] User profile analysis", {
    ...
    avoidsFamily,
    avoidsChildrens,
    topKeywords: Array.from(pref.keywords.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([k, v]) => `${k}(${v.toFixed(1)})`),
    topGenreCombos: Array.from(pref.genreCombos.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([k, v]) => `${k}(${v.toFixed(1)})`),
    topDirectors: Array.from(pref.directors.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([d, v]) => `${d}(${v.toFixed(1)})`),
    topCast: Array.from(pref.cast.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
  ...
  const filmsForSubgenreAnalysis = mappedFilmsForAnalysis.map((f, idx) => {
    ...
    return {
      ...
      keywords:
        (cached as any)?.keywords?.keywords?.map((k: any) => k.name) ||
  ...
  const crossGenrePatterns = analyzeCrossGenrePatterns(
    filmsForSubgenreAnalysis,
  );
  ...
  console.log("[Suggest] Subgenre analysis complete", {
    ...
    exampleAvoidances: Array.from(subgenrePatterns.entries())
      ...
      .map(
        ([genre, p]) =>
          `${genre}: avoids ${Array.from(p.avoidedSubgenres).slice(0, 2).join(", ")}`,
      ),
  });
  ...
  if (
    params.enhancedProfile?.adjacentGenres &&
    params.enhancedProfile?.recentGenres
  ) {
  ...
  const watchlistGenreSet = new Set(
    params.enhancedProfile?.watchlistGenres ?? [],
  );
  const watchlistKeywordSet = new Set(
    params.enhancedProfile?.watchlistKeywords ?? [],
  );
  const watchlistDirectorSet = new Set(
  ...
  const pool = await mapLimit(
    ...
    async (cid) => {
      ...
      if (
        params.excludeGenres &&
        feats.genres.some((g) => params.excludeGenres!.has(g.toLowerCase()))
      ) {
        return null;
      }
      ...
      const nicheProfile = {
        nichePreferences: {
          ...
          likesStandUp: Array.from(pref.keywords.keys()).some(
            (k) =>
              k.toLowerCase().includes("stand-up") ||
              k.toLowerCase().includes("stand up"),
          ),
          likesFoodDocs: Array.from(pref.keywords.keys()).some(
            (k) =>
              k.toLowerCase().includes("food") ||
              k.toLowerCase().includes("cooking"),
          ),
          likesTravelDocs: Array.from(pref.keywords.keys()).some(
            (k) =>
      ...
      const computeMetadataCompleteness = (): number => {
        ...
        const hasOverviewLocal = Boolean(
          m.overview && m.overview.trim().length > 20,
        );
        const hasTrailerLocal = Boolean(
          (m as any).videos?.results?.some(
            (v: any) => v.site === "YouTube" && v.type === "Trailer",
          ),
        );
        const hasVotesLocal = Boolean(
          (m as any).vote_count && (m as any).vote_count >= 50,
        );
      ...
      if (qualityPenalty < 0) {
        ...
        if (qualityNotes.length) {
          ...
          if (strongConsensus) {
            ...
            reasons.push(
              `Limited metadata (${note}) — confidence slightly reduced`,
            );
          }
      ...
      if (watchlistIntent) {
        ...
        const when = watchlistIntent.ageText
          ? `, added ${watchlistIntent.ageText}`
          : "";
        reasons.push(
          `On your Letterboxd watchlist (${watchlistIntent.label}${when}) — honoring your intent`,
        );
      }
      ...
      if (adjacentBoosts.size > 0) {
        ...
        if (maxAdjBoost > 0) {
          ...
          if (maxAdjBoost > 0) {
            score += maxAdjBoost;
            reasons.push(
              `Matches your learned preference for ${boostedGenre} after recent watches`,
            );
          }
        }
      }
      ...
      if (feats.genreCombo && pref.genreCombos.has(feats.genreCombo)) {
        ...
        reasons.push(
          `Matches your specific taste in ${feats.genres.join(" + ")} films (${comboCountRounded} highly-rated similar ${comboCountRounded === 1 ? "film" : "films"}) — ${strength} signal`,
        );
      } else {
        ...
        if (gHits.length) {
          ...
          reasons.push(
            `Matches your taste in ${gHits.slice(0, 3).join(", ")} (${genreCountRounded} similar ${genreCountRounded === 1 ? "film" : "films"}) — ${strength} signal`,
          );

          // Watchlist intent reason if genre aligns with user's watchlist
          const watchlistGenreHits = gHits.filter((g) =>
            watchlistGenreSet.has(g),
          );
          if (watchlistGenreHits.length > 0) {
            reasons.push(
              `On your watchlist: ${watchlistGenreHits.slice(0, 2).join(", ")}`,
            );
          }
        }
      }
      ...
      if (dHits.length) {
        ...
        reasons.push(
          `Directed by ${dHits.slice(0, 2).join(", ")} — you've ${dirQuality} ${dirCountRounded} ${dirCountRounded === 1 ? "film" : "films"} by ${dHits.length === 1 ? "this director" : "these directors"} — ${strength} signal`,
        );
        ...
        const similarDirectors: Array<{
          director: string;
          likedDirector: string;
          sharedThemes: string[];
        }> = [];
        ...
        for (const candidateDir of feats.directors) {
          ...
          for (const [
            likedDir,
            dirKeywords,
          ] of pref.directorKeywords.entries()) {
            const sharedKeywords = Array.from(dirKeywords).filter((k) =>
              candidateKeywords.has(k),
            );
        ...
        if (similarDirectors.length) {
          ...
          reasons.push(
            `Similar to ${firstMatch.likedDirector} you love — shares themes like ${firstMatch.sharedThemes.slice(0, 2).join(", ")} (borrowed confidence)`,
          );
        }
      }
      ...
      if (cHits.length) {
        ...
        reasons.push(
          `Stars ${cHits.slice(0, 3).join(", ")} — ${cHits.length} cast ${cHits.length === 1 ? "member" : "members"} you've liked before — ${strength} signal`,
        );
      } else {
        // Check for similar actors (actors who work in the same subgenres)
        const similarCast: Array<{
          actor: string;
          likedActor: string;
          sharedThemes: string[];
        }> = [];
        ...
        for (const candidateActor of feats.cast) {
          ...
          for (const [
            likedActor,
            actorKeywords,
          ] of pref.castKeywords.entries()) {
            const sharedKeywords = Array.from(actorKeywords).filter((k) =>
              candidateKeywords.has(k),
            );
            if (sharedKeywords.length >= 2) {
              similarCast.push({
        ...
        if (similarCast.length) {
          ...
          reasons.push(
            `Similar to ${firstMatch.likedActor} you enjoy — works in ${firstMatch.sharedThemes.slice(0, 2).join(", ")} themes (borrowed confidence)`,
          );
        }
      }
      ...
      const studioHits = feats.productionCompanies.filter((s) =>
        pref.productionCompanies.has(s),
      );
      const hasEnhancedStudioData =
        params.enhancedProfile?.topStudios &&
        params.enhancedProfile.topStudios.length > 0;
      ...
      if (studioHits.length && !hasEnhancedStudioData) {
        ...
        const isNotableStudio = notableStudios.some((n) =>
          topStudio.includes(n),
        );

        if (isNotableStudio) {
          reasons.push(
            `From ${topStudio} — you've loved ${studioCountRounded} ${studioCountRounded === 1 ? "film" : "films"} from this studio`,
          );
        } else {
          reasons.push(
            `From ${studioHits.slice(0, 2).join(", ")} — studios you enjoy`,
          );
        }
      }
      ...
      if (params.enhancedProfile?.topActors) {
        ...
        const matchedActors = params.enhancedProfile.topActors.filter((a) =>
          actorIds.has(a.id),
        );
        ...
        if (matchedActors.length > 0) {
          ...
          if (actorCount === 1) {
            reasons.push(
              `Stars ${topActor.name} — one of your favorite actors`,
            );
          } else {
            reasons.push(
              `Stars ${matchedActors
                .slice(0, 2)
                .map((a) => a.name)
                .join(" and ")} — ${actorCount} actors you love`,
            );
          }
        }
      }
      ...
      if (params.enhancedProfile?.topDecades && m.release_date) {
        ...
        if (!isNaN(year)) {
          ...
          const matchedDecade = params.enhancedProfile.topDecades.find(
      ...
      if (params.enhancedProfile?.avoidGenres) {
        const avoidedGenreIds = new Set(
          params.enhancedProfile.avoidGenres.map((g) => g.id),
        );
        const matchedAvoidGenres = feats.genreIds.filter((id) =>
          avoidedGenreIds.has(id),
        );

        if (matchedAvoidGenres.length > 0) {
          const genrePenalty =
            matchedAvoidGenres.length * weights.avoidGenrePenalty;
          totalPenalty += genrePenalty;
          const avoidedGenre = params.enhancedProfile.avoidGenres.find(
            (g) => g.id === matchedAvoidGenres[0],
          );
          penaltyReasons.push(
            `Contains ${avoidedGenre?.name || "genre"} you typically avoid`,
          );
        }
      }

      if (params.enhancedProfile?.avoidKeywords) {
        const avoidedKeywordIds = new Set(
          params.enhancedProfile.avoidKeywords.map((k) => k.id),
        );
        const matchedAvoidKeywords = feats.keywordIds.filter((id) =>
          avoidedKeywordIds.has(id),
        );

        if (matchedAvoidKeywords.length > 0) {
          const keywordPenalty =
            matchedAvoidKeywords.length * weights.avoidKeywordPenalty;
          totalPenalty += keywordPenalty;
          const avoidedKeyword = params.enhancedProfile.avoidKeywords.find(
            (k) => k.id === matchedAvoidKeywords[0],
          );
          penaltyReasons.push(
            `Has themes (${avoidedKeyword?.name || "keyword"}) you dislike`,
          );
        }
      }

      if (params.enhancedProfile?.avoidDirectors) {
        const avoidedDirectorIds = new Set(
          params.enhancedProfile.avoidDirectors.map((d) => d.id),
        );
        const matchedAvoidDirectors = feats.directorIds.filter((id) =>
          avoidedDirectorIds.has(id),
        );

        if (matchedAvoidDirectors.length > 0) {
          const directorPenalty =
            matchedAvoidDirectors.length * weights.avoidDirectorPenalty;
          totalPenalty += directorPenalty;
          const avoidedDirector = params.enhancedProfile.avoidDirectors.find(
            (d) => d.id === matchedAvoidDirectors[0],
          );
          penaltyReasons.push(
            `Directed by ${avoidedDirector?.name || "director"} whose films you don't enjoy`,
          );
        }
      }
      ...
      if (subgenrePatterns && subgenrePatterns.size > 0) {
        ...
        for (const subgenre of detectedSubgenres) {
          ...
          if (params.allowSubgenres?.includes(subgenre)) {
            ...
            score += boost;
            const prettyName = subgenre
              .replace(/^[A-Z]+_/, "")
              .replace(/_/g, " ")
              .toLowerCase();
            reasons.push(`Matches your selected ${prettyName} sub-genre`);
            console.log(
              `[UserSelectedSubgenre] Boosted "${m.title}" by ${boost.toFixed(2)} for ${subgenre}`,
            );
          ...
          for (const [parentGenre, pattern] of subgenrePatterns.entries()) {
            ...
            if (
              pattern.preferredSubgenres.has(subgenre) ||
              (subInfo &&
                subInfo.weight > 2.0 &&
                subInfo.liked > subInfo.watched - subInfo.liked)
            ) {
      ...
      if (params.featureFeedback) {
        ...
        if (ff.avoidActors && ff.avoidActors.length > 0) {
          ...
          const movieCastIds = (m.credits?.cast || [])
            .slice(0, 5)
            .map((c: any) => c.id);
          ...
          if (matchedAvoidActors.length > 0) {
            ...
            const leadActor = matchedAvoidActors.find(
              (a) => movieCastIds.indexOf(a.id) < 2,
            );
            const supportingActors = matchedAvoidActors.filter(
              (a) => movieCastIds.indexOf(a.id) >= 2,
            );
            ...
            if (leadActor) {
              ...
              const leadPenalty = -applySoftCap(
                effectiveWeight * hardnessMultiplier,
                4,
              );
              actorPenalty += leadPenalty;
              const countLabel =
                leadActor.count === 1 ? "once" : `${leadActor.count} times`;
              const hardnessLabel =
                leadActor.count >= 3 ? "hard avoid" : "soft avoid";
              penaltyReasons.push(
                `Stars ${leadActor.name} — ${hardnessLabel}; you've skipped their films ${countLabel}`,
              );
            }
        ...
        if (ff.avoidKeywords && ff.avoidKeywords.length > 0) {
          const avoidedKeywordMap = new Map(
            ff.avoidKeywords.map((k) => [k.id, k]),
          );
          ...
          if (matchedKeywords.length > 0) {
            ...
            const topKeyword = matchedKeywords.sort(
              (a, b) => b.weight - a.weight,
            )[0];
            const countLabel =
              topKeyword.count === 1 ? "before" : `${topKeyword.count} times`;
            const hardnessLabel =
              topKeyword.count >= 3 ? "hard avoid" : "soft avoid";
            penaltyReasons.push(
        ...
        if (
          ff.avoidFranchises &&
          ff.avoidFranchises.length > 0 &&
          feats.collection
        ) {
          const avoidedFranchise = ff.avoidFranchises.find(
            (f) => f.id === feats.collection!.id,
          );
          if (avoidedFranchise) {
            ...
            const franchisePenalty = -applySoftCap(
              avoidedFranchise.weight * confidence * 1.2,
              6,
            );
            totalPenalty += franchisePenalty;
            const countLabel =
              avoidedFranchise.count === 1
                ? ""
                : ` (${avoidedFranchise.count} skips)`;
            penaltyReasons.push(
        ...
        if (ff.preferActors && ff.preferActors.length > 0) {
          const preferredActorMap = new Map(
            ff.preferActors.map((a) => [a.id, a]),
          );
          const movieCastIds = (m.credits?.cast || [])
            .slice(0, 5)
            .map((c: any) => c.id);
          ...
          if (matchedPreferActors.length > 0) {
            const leadActor = matchedPreferActors.find(
              (a) => movieCastIds.indexOf(a.id) < 2,
            );
            ...
            if (leadActor) {
              ...
              const countLabel =
                leadActor.count === 1 ? "" : ` (${leadActor.count}× thumbs up)`;
              reasons.push(
                `Stars ${leadActor.name}${countLabel} — you love their work`,
              );
            } else {
              const topActor = matchedPreferActors[0];
              const confidence = confidenceFromCount(topActor.count);
              actorBoost += applySoftCap(
                topActor.weight * confidence * 0.6,
                2.0,
              );
              reasons.push(
                `Features ${topActor.name} — you've enjoyed their films`,
              );
            }
        ...
        if (ff.preferKeywords && ff.preferKeywords.length > 0) {
          const preferredKeywordMap = new Map(
            ff.preferKeywords.map((k) => [k.id, k]),
          );
          const matchedKeywords = feats.keywordIds
        ...
        if (ff.avoidDirectors && ff.avoidDirectors.length > 0) {
          const avoidedDirectorMap = new Map(
            ff.avoidDirectors.map((d) => [d.id, d]),
          );
          ...
          if (matchedAvoidDirectors.length > 0) {
            ...
            const directorPenalty = -applySoftCap(
              director.weight * confidence * 1.5,
              4.0,
            );
            totalPenalty += directorPenalty;
            const countLabel =
              director.count === 1 ? "once" : `${director.count} times`;
            penaltyReasons.push(
        ...
        if (ff.preferDirectors && ff.preferDirectors.length > 0) {
          const preferredDirectorMap = new Map(
            ff.preferDirectors.map((d) => [d.id, d]),
          );
          ...
          if (matchedPreferDirectors.length > 0) {
            ...
            const confidence = confidenceFromCount(director.count);
            const directorBoost = applySoftCap(
              director.weight * confidence * 1.5,
              4.0,
            );
            score += directorBoost;
            const countLabel =
              director.count === 1 ? "" : ` (${director.count}× chosen)`;
            reasons.push(
        ...
        if (ff.avoidGenres && ff.avoidGenres.length > 0) {
          ...
          if (matchedAvoidGenres.length > 0) {
            ...
            totalPenalty += genrePenalty;
            const topGenre = matchedAvoidGenres.sort(
              (a, b) => b.weight - a.weight,
            )[0];
            penaltyReasons.push(
        ...
        if (ff.preferGenres && ff.preferGenres.length > 0) {
          const preferredGenreMap = new Map(
            ff.preferGenres.map((g) => [g.id, g]),
          );
          ...
          if (matchedPreferGenres.length > 0) {
            ...
            score += genreBoost;
            const topGenre = matchedPreferGenres.sort(
              (a, b) => b.weight - a.weight,
            )[0];
            reasons.push(
        ...
        if (
          ff.avoidSubgenres &&
          ff.avoidSubgenres.length > 0 &&
          candidateSubgenres.size > 0
        ) {
          const matchedAvoidSubgenres = ff.avoidSubgenres.filter((s) =>
            candidateSubgenres.has(s.key),
          );
          ...
          if (matchedAvoidSubgenres.length > 0) {
            ...
            const topSubgenre = matchedAvoidSubgenres.sort(
              (a, b) => b.weight - a.weight,
            )[0];
            const prettyName = topSubgenre.key
              .replace(/^[A-Z]+_/, "")
              .replace(/_/g, " ")
              .toLowerCase();
            penaltyReasons.push(
        ...
        if (
          ff.preferSubgenres &&
          ff.preferSubgenres.length > 0 &&
          candidateSubgenres.size > 0
        ) {
          const matchedPreferSubgenres = ff.preferSubgenres.filter((s) =>
            candidateSubgenres.has(s.key),
          );
          ...
          if (matchedPreferSubgenres.length > 0) {
            ...
            score += subgenreBoost;
            const topSubgenre = matchedPreferSubgenres.sort(
              (a, b) => b.weight - a.weight,
            )[0];
            const prettyName = topSubgenre.key
              .replace(/^[A-Z]+_/, "")
      ...
      const recentGenreMatches = feats.genres.filter((g) =>
        pref.recentGenres.has(g),
      );
      if (recentGenreMatches.length) {
        recentBoost += 0.5 * recentGenreMatches.length;
        recentMatches.push(
          `similar to recent ${recentGenreMatches.slice(0, 2).join("/")} films`,
        );
      }

      const recentDirectorMatches = feats.directors.filter((d) =>
        pref.recentDirectors.has(d),
      );
      if (recentDirectorMatches.length) {
        recentBoost += 1.0 * recentDirectorMatches.length;
        recentMatches.push(
          `from ${recentDirectorMatches[0]} you recently enjoyed`,
        );
      }

      const recentCastMatches = feats.cast
        .filter((c) => pref.recentCast.has(c))
        .slice(0, 2);
      ...
      const recentKeywordMatches = feats.keywords
        .filter((k) => pref.recentKeywords.has(k))
        .slice(0, 3);
      if (recentKeywordMatches.length >= 2) {
        recentBoost += 0.4 * recentKeywordMatches.length;
        recentMatches.push(
          `explores ${recentKeywordMatches.slice(0, 2).join("/")} themes from recent favorites`,
        );
      }

      const recentStudioMatches = feats.productionCompanies.filter((s) =>
        pref.recentStudios.has(s),
      );
      if (recentStudioMatches.length) {
        recentBoost += 0.6 * recentStudioMatches.length;
        recentMatches.push(
          `from ${recentStudioMatches[0]} you recently enjoyed`,
        );
      }
      ...
      if (recentBoost > 0) {
        ...
        reasons.push(
          `Based on recent watches: ${recentMatches.slice(0, 2).join("; ")}`,
        );
      }
      ...
      const contributingFilms: Record<
        string,
      ...
      const dHitsForLookup = feats.directors.filter((d) =>
        pref.directors.has(d),
      );
      ...
      const studioHitsForLookup = feats.productionCompanies.filter((s) =>
        pref.productionCompanies.has(s),
      );
      ...
      if (sourceMeta) {
        ...
        const multiSourceBoost =
          sources.length > 1
            ? 1 + Math.min((sources.length - 1) * 0.02, 0.06)
            : 1;
        reliabilityMultiplier = Math.min(
          1.12,
          Math.max(0.9, avgReliability * consensusBoost * multiSourceBoost),
        );
 ...
 */
export async function getAdaptiveExplorationRate(
  userId: string,
): Promise<number> {
  ...
  try {
    ...
    if (!data || data.exploratory_films_rated < 10) {
      ...
      console.log(
        "[AdaptiveExploration] Using default rate (insufficient data)",
        {
...
export async function updateExplorationFeedback(
  ...
  try {
    ...
    if (!current) {
      ...
      const newAvg =
        (current.exploratory_avg_rating * current.exploratory_films_rated +
          rating) /
        newCount;
    ...
    for (const filmGenre of filmGenres) {
      ...
      for (const topGenre of userTopGenres.slice(0, 3)) {
        ...
        if (existing) {
          ...
          const newAvg =
            (existing.avg_rating * existing.rating_count + rating) / newCount;
          const successCount =
 ...
 */
export async function getRepeatSuggestionStats(
  userId: string,
  lookbackDays = 30,
): Promise<{
  ...
  try {
    ...
    const repeatRate =
      uniqueSuggestions > 0
        ? (totalExposures - uniqueSuggestions) / totalExposures
        : 0;
...
> {
  ...
  try {
    ...
    if (error) {
      console.error(
        "[CounterfactualReplay] Error fetching exposure data:",
        error,
      );
      return [];
    }
    ...
    return exposures.map((exp) => {
      ...
      return {
        ...
        feedbackType:
          (feedback?.type as "positive" | "negative" | null) || null,
...
export const makePairId = (a: number, b: number) =>
...
export const reasonTypeTags = (reasons: string[]) => {
  ...
  for (const r of reasons) {
    ...
    if (lower.includes("genre") || lower.includes("taste in"))
      tags.add("genre");
...
export function findPairwiseCandidate<T extends PairwiseCandidate>(
  list: T[],
  history: Set<string>,
): { a: T; b: T } | null {
