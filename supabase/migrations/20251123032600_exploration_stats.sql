...
-- Policies
drop policy if exists "Users can view own exploration stats" on user_exploration_stats;
create policy "Users can view own exploration stats"
  on user_exploration_stats for select
  using (auth.uid() = user_id);
drop policy if exists "Users can insert own exploration stats" on user_exploration_stats;
create policy "Users can insert own exploration stats"
  on user_exploration_stats for insert
  with check (auth.uid() = user_id);
drop policy if exists "Users can update own exploration stats" on user_exploration_stats;
