...
import { upsertTmdbCacheAction } from "@/app/actions/enrichment";
...
export default function LibraryPage() {
  ...
  const [trailerKeys, setTrailerKeys] = useState<Map<number, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      console.log("[Library] start loading mappings", {
        uid,
  ...
  const mappedIds = useMemo(
    () => Array.from(new Set(Array.from(mappings.values()))),
    [mappings],
  );
  ...
  return (
    <AuthGate>
      ...
      <p className="text-sm text-gray-700 mb-4">
        Your watched films. Use this page to review mapping accuracy and edit
        any mismatched titles.
      </p>
      {error && <p className="text-sm text-red-600 mb-4">{error}</p>}
      {loadingFilms && (
        <p className="text-sm text-gray-600">
          Loading your library from database…
        </p>
      )}
      <MovieGrid
        films={gridFilms}
...
function MovieGrid({
  ...
  uid: string | null;
  mappedIds: number[];
  trailerKeys: Map<number, string>;
  onMappingChange: (uri: string, id: number) => void;
  onDeleteMapping: (uri: string) => void;
}) {
  const { posters, backdrops, loading } = usePostersSWR(mappedIds);
  if (!films.length)
    return (
      <p className="text-sm text-gray-600">
        No films loaded yet. Import your data first.
      </p>
    );
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
      {films.map((f) => (
        <MovieCard
          ...
          posters={posters}
          backdrops={backdrops}
          trailerKey={f.tmdbId ? trailerKeys.get(f.tmdbId) : undefined}
          loading={loading}
...
function MovieCard({
  ...
  const posterPath =
    mapped && film.tmdbId ? (posters[film.tmdbId] ?? null) : null;
  const backdropPath =
    mapped && film.tmdbId ? (backdrops[film.tmdbId] ?? null) : null;
  ...
  const applyMapping = async (tmdbId: number) => {
    ...
    try {
      ...
      if (chosen) {
        const { error } = await upsertTmdbCacheAction(chosen);
        if (error) {
          console.error("[Library] Cache upsert failed", { tmdbId, error });
        }
      }
  ...
  return (
    <div className="group relative bg-white rounded shadow-sm border overflow-hidden">
      <div className="aspect-[2/3] bg-gray-200 relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-2">
            <p
              className="text-[11px] text-white font-medium line-clamp-2"
              title={film.title}
            >
              {film.title}
            </p>
            <p className="text-[10px] text-gray-200">
              {film.year || "—"} · Watches: {film.watchCount ?? 0}
            </p>
          </div>
        </div>
      </div>
      <div className="p-2">
        ...
        <p
          className={`text-[10px] mt-1 ${mapped ? "text-green-700" : "text-red-700"}`}
        >
          {mapped ? `Mapped (${film.tmdbId})` : "Unmapped"}
        </p>
        <div className="mt-2 flex gap-1">
          <button
            className="flex-1 bg-gray-100 hover:bg-gray-200 text-[10px] rounded py-1"
          ...
          {mapped && (
            <button
              className="flex-1 bg-red-50 hover:bg-red-100 text-[10px] rounded py-1"
      ...
      {open && (
        <div className="absolute inset-0 bg-white/95 backdrop-blur-sm p-2 flex flex-col">
          <div className="flex items-center justify-between mb-2">
            ...
            <button className="text-[10px] px-1" onClick={() => setOpen(false)}>
              ✕
            </button>
          </div>
          <input
          ...
          <input
            ...
            value={searchYear ?? ""}
            onChange={(e) =>
              setSearchYear(e.target.value ? Number(e.target.value) : undefined)
            }
          />
          <button
            disabled={searching}
            onClick={runSearch}
            className="text-[10px] bg-blue-600 text-white rounded py-1 mb-2 disabled:opacity-50"
          >
          ...
          <div className="flex-1 overflow-auto space-y-1">
            ...
            {results && results.length === 0 && (
              <p className="text-[10px] text-gray-500">No results</p>
            )}
          </div>
        </div>
      )}
...
function postersGlobal() {
  return posterState.current;
}
function setPostersGlobal(p: Record<number, string | null>) {
  posterState.current = p;
}

const backdropState: { current: Record<number, string | null> } = {
  current: {},
};
function backdropsGlobal() {
  return backdropState.current;
}
function setBackdropsGlobal(p: Record<number, string | null>) {
  backdropState.current = p;
}
