...
export async function GET(req: Request) {
  try {
    ...
    console.log("[TrendingAPI] Cache check", {
    ...
    const freshEnough =
      ...
      Date.now() - new Date(existing[0].updated_at).getTime() < ONE_DAY_MS;
    if (freshEnough) {
      console.log("[TrendingAPI] Using cache");
      return NextResponse.json({
        ok: true,
        ids: existing.map((r) => r.tmdb_id),
        source: "cache",
      });
    }
    ...
    if (!r.ok) {
      ...
      console.error("[TrendingAPI] TMDB request failed", {
        status: r.status,
        body: txt,
      });
      return NextResponse.json(
        { error: "TMDB trending request failed", status: r.status, body: txt },
        { status: 502 },
      );
    }
    const j = await r.json();
    const results = Array.isArray(j?.results) ? j.results.slice(0, limit) : [];
    console.log("[TrendingAPI] Got results from TMDB", {
      count: results.length,
    });
    const rows: Array<{ period: string; tmdb_id: number; rank: number }> =
